...
def numpyro_poisson_likelihood(
    ...
    def _log_likelihood_core(
        ...
        for batched_data, batched_log_ref_priors, batched_masks in zip(
            data_group_, log_ref_priors_group_, masks_group_
        ):
            ...
            batched_model_log_prob: Array = model_instance.log_prob(safe_data)

            # Importance weighting: log ρ(θ | Λ) - log π_n(θ)
            batched_log_prob = batched_model_log_prob - batched_log_ref_priors
            batched_log_prob = jnp.where(batched_masks, batched_log_prob, -jnp.inf)

            # For each event: log ∑_samples exp(...)
            # log_prob_sum shape: (n_events,)
    ...
    def log_likelihood_fn(
        ...
        log_likelihood = _log_likelihood_core(
            data_group,
