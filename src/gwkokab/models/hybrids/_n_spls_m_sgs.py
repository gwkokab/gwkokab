    ...
    **params: Array,
    ...
    class _NSmoothingPowerlawMGaussian(Distribution):
        ...
        pytree_data_fields = (
            ...
            + tuple(f"a1_loc_pl_{i}" for i in range(N_pl))
            + tuple(f"a1_scale_g_{i}" for i in range(N_g))
            + tuple(f"a1_scale_pl_{i}" for i in range(N_pl))
            + tuple(f"a2_loc_g_{i}" for i in range(N_g))
            + tuple(f"a2_loc_pl_{i}" for i in range(N_pl))
            + tuple(f"a2_scale_g_{i}" for i in range(N_g))
            + tuple(f"a2_scale_pl_{i}" for i in range(N_pl))
            + tuple(f"t1_loc_g_{i}" for i in range(N_g))
            + tuple(f"t1_loc_pl_{i}" for i in range(N_pl))
            + tuple(f"t1_scale_g_{i}" for i in range(N_g))
            + tuple(f"t1_scale_pl_{i}" for i in range(N_pl))
            + tuple(f"t2_loc_g_{i}" for i in range(N_g))
            + tuple(f"t2_loc_pl_{i}" for i in range(N_pl))
            + tuple(f"t2_scale_g_{i}" for i in range(N_g))
            + tuple(f"t2_scale_pl_{i}" for i in range(N_pl))
        )
