...
"""Shared definitions for the multisource analysis family.

MultiSourceModelCore is the family half of the analysis matrix: it names the event
coordinates the data must supply (:attr:`~MultiSourceModelCore.parameters`) and the flat
list of population hyper-parameters to be inferred
(:attr:`~MultiSourceModelCore.model_parameters`). Both are derived from the ``use_*``
flags, so switching a physical parameter on at the command line extends the parameter
list automatically.

The model is :func:`~gwkokab.models.hybrids.MultiSourceModel`, built from ``N_spl``
smoothed power-law, ``N_bpl`` smoothed broken power-law, ``N_gpl`` Gaussian power-law
and ``N_gg`` Gaussian-Gaussian mass components. Hyper-parameter names carry the
component tags ``spl``, ``bpl``, ``gpl`` and ``gg``, giving forms like ``alpha_pl_0``.

This module is mixed with a data-representation base and a sampler mixin to form a
complete analysis; see the ``discrete`` and ``analytical_gwalk`` modules alongside it.
"""
...
def where_fns_list(
    ...
    if use_beta_spin_magnitude:
        ...
        def positive_concentration(**kwargs) -> Array:
            ...
            screens the pairs over every component family.
...
class MultiSourceModelCore:
    """Family half of the multisource analysis.
    ...
    :func:`~gwkokab.models.hybrids.MultiSourceModel`.
    """
