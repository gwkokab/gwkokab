...
def poisson_mean_parser(
    ...
    with open(filepath, "r") as file:
        pmean_json = json.load(file)
    proposal_distribution = []
    proposal_distribution_dict: Dict[
        str, Union[Literal["self"], List[Dict[str, Any]]]
    ] = pmean_json.pop("proposal_dists")
    for per_component_dist in proposal_distribution_dict:
        if isinstance(per_component_dist, str):
            ...
            proposal_distribution.append("self")
        elif isinstance(per_component_dist, list):
            error_if(
                not all(
                    isinstance(dist_dict, dict) for dist_dict in per_component_dist
                ),
                msg="The proposal distribution must be a list of dictionaries.",
            )
            proposal_distribution.append(
                JointDistribution(
