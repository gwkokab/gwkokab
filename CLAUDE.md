...
CI (`.github/workflows/ci.yml`) runs prek → tests (py3.11 & 3.13) → docs, with `JAX_ENABLE_X64=1`, `JAX_PLATFORMS=cpu`, `XLA_FLAGS=--xla_force_host_platform_device_count=8`. `pyproject.toml` sets `filterwarnings = ["error", ...]`, so a new warning fails tests.
...
Hyper-parameter names follow `<name>_<component>_<index>`, where component is `pl` (power-law) or `g` (Gaussian): `alpha_pl_0`, `mmax_pl_1`, `spin_1z_loc_g_0`, `log_rate_2`. `analysis/utils/common.py::expand_arguments` generates them.
...
See `docs/source/examples/ecc_plus_spin/` for a complete worked run: `synthetic_events_<family>` (draw a population) → `synthetic_discrete_pe` or `synthetic_analytical_gwalk_pe` (add measurement error, write per-event files) → `discrete_<family>` / `analytical_gwalk_<family>` (inference) → `gwk_report`.
...
- Test files must be named `test_*.py` (enforced by the `name-tests-test` hook). Tests use `pytest` + `chex` + `absl.testing.parameterized`.
