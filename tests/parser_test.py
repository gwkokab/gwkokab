...
def test_spin_model() -> None:
    ...
    assert dic["spin_model"]["params"]["alpha"] == 0.8
    assert dic["spin_model"]["params"]["beta"] == 0.8
    assert dic["spin_model"]["params"]["chimax"] == 1.0
    assert dic["spin_model"]["params"]["name"] == "Wysocki2019SpinModel_test"
    assert dic["spin_model"]["config_vars"] == ["alpha", "beta"]
    assert dic["spin_model"]["col_names"] == ["chi_source"]
