...
@jit
def interpolate_hdf5(m1, m2, file_path:str = "./vt_1_200_1000.hdf5"):
    """
    Interpolates the VT values from an HDF5 file based on given m1 and m2 coordinates.
    
    Parameters:
        m1 (float): The m1 coordinate.
        m2 (float): The m2 coordinate.
        file_path (str, optional): The path to the HDF5 file. Defaults to "./vt_1_200_1000.hdf5".
    
    Returns:
        interpolator: The interpolated VT value.
    """
...
def bispline_interp(xnew, ynew, xp, yp, zp):
    ...
    https://github.com/google/jax/discussions/10689
    Parameters:
    - xnew (ndarray): 1D vector of x-coordinates where to perform predictions.
    - ynew (ndarray): 1D vector of y-coordinates where to perform predictions.
    - xp (ndarray): 1D vector of original grid points along the x-axis.
    - yp (ndarray): 1D vector of original grid points along the y-axis.
    - zp (ndarray): 2D array of original values of functions, where zp[i,j] is the value at xp[i], yp[j].

    Returns:
    - ndarray: Interpolated values at the specified coordinates (xnew, ynew).
    """
